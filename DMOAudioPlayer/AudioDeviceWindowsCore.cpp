...
AudioDeviceWindowsCore::CoreAudioIsSupported() 
{
    ...
    if (isVistaRTMorXP != 0) {
        LOG_VERBOSE( L"*** Windows Core Audio is only supported on Vista SP1 or later => will revert to the Wave API ***");
        return false;
    }
    ...
    if (FAILED(hr)) {
        ...
        LOG_VERBOSE( L"AudioDeviceWindowsCore::CoreAudioIsSupported() CoCreateInstance(MMDeviceEnumerator) failed (hr=" << hr << L")");
        ...
        LOG_VERBOSE( L"AudioDeviceWindowsCore::CoreAudioIsSupported()" << L" CoCreateInstance(MMDeviceEnumerator) succeeded (hr=" << hr << L")");
        SAFE_RELEASE(pIMMD);
    }
    ...
    if (MMDeviceIsAvailable) {
        ...
        for (uint16_t i = 0; i < numDevsRec; i++) {
            ...
            if (ok)
                LOG_WARNING( L"AudioDeviceWindowsCore::CoreAudioIsSupported() Failed to use Core Audio Recording for device id=" << i);
        }
        ...
        for (uint16_t i = 0; i < numDevsPlay; i++) {
            ...
            if (ok) {
                LOG_WARNING( L"AudioDeviceWindowsCore::CoreAudioIsSupported() Failed to use Core Audio Playout for device id=" << i);
            }
        }
    ...
    if (coreAudioIsSupported) {
        LOG_VERBOSE( L"*** Windows Core Audio is supported ***");
    }
    else {
        LOG_VERBOSE( L"*** Windows Core Audio is NOT supported" << L" => will revert to the Wave API ***");
    }
...
{
    USES_CONVERSION;
    LOG_TRACE( A2W(__FUNCTION__) << L" created");
    ...
    if (!_avrtLibrary) {
        ...
        if (_avrtLibrary) {
            ...
            LOG_VERBOSE( L"AudioDeviceWindowsCore::AudioDeviceWindowsCore() The Avrt DLL module is now loaded");
            ...
            if (_PAvRevertMmThreadCharacteristics && _PAvSetMmThreadCharacteristicsA && _PAvSetMmThreadPriority) {
                LOG_VERBOSE( L"AudioDeviceWindowsCore::AudioDeviceWindowsCore()" << L" AvRevertMmThreadCharacteristics() is OK");
                LOG_VERBOSE( L"AudioDeviceWindowsCore::AudioDeviceWindowsCore()" << L" AvSetMmThreadCharacteristicsA() is OK");
                LOG_VERBOSE( L"AudioDeviceWindowsCore::AudioDeviceWindowsCore()" << L" AvSetMmThreadPriority() is OK");
...
AudioDeviceWindowsCore::~AudioDeviceWindowsCore() 
{
    ...
    LOG_TRACE( A2W(__FUNCTION__) << L" destroyed");
    ...
    if (_avrtLibrary) 
    {
        ...
        if (!freeOK) {
            LOG_WARNING( L"AudioDeviceWindowsCore::~AudioDeviceWindowsCore() failed to free the loaded Avrt DLL module correctly");
        }
        else {
            LOG_WARNING( L"AudioDeviceWindowsCore::~AudioDeviceWindowsCore() the Avrt DLL module is now unloaded");
        }
    }
}
...
int32_t AudioDeviceWindowsCore::InitSpeaker() {
    ...
    if (m_usingOutputDeviceIndex) {
        ...
        if (m_outputDeviceIndex > (nDevices - 1)) {
            LOG_ERROR( L"current device selection is invalid => unable to initialize");
            return -1;
        }
    }
    ...
    if (ret != 0 || (m_ptrDeviceOut == NULL)) {
        LOG_ERROR( L"failed to initialize the rendering enpoint device");
        m_ptrDeviceOut.Release();
        return -1;
    }
    ...
    {
        LOG_ERROR( L"failed to initialize the render manager");
        pManager.Release();
        return -1;
    }
    ...
    {
        LOG_ERROR( L"failed to initialize the render simple volume");
        pManager.Release();
        m_ptrRenderSimpleVolume.Release();
        return -1;
    }
...
int32_t AudioDeviceWindowsCore::InitMicrophone() {
    ...
    if (ret != 0 || (m_ptrDeviceIn == NULL)) {
        LOG_ERROR( L"failed to initialize the capturing enpoint device");
        m_ptrDeviceIn.Release();
        return -1;
    }
    ...
    if (ret != 0 || m_ptrCaptureVolume == NULL) {
        LOG_ERROR( L"failed to initialize the capture volume");
        m_ptrCaptureVolume.Release();
        return -1;
    }
...
int32_t AudioDeviceWindowsCore::SetMicrophoneVolume(uint32_t volume) {
    LOG_VERBOSE( L"AudioDeviceWindowsCore::SetMicrophoneVolume(volume=" << volume << L")" );
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(uint16_t index) 
{
    ...
    if (index < 0 || index >(nDevices - 1)) {
        LOG_ERROR( L"device index is out of range [0," << (nDevices - 1) << L"]");
        return -1;
    }
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(AudioDeviceModule::WindowsDeviceType device) 
{
    ...
    // Get the endpoint device's friendly-name
    if (_GetDeviceName(m_ptrDeviceOut, szDeviceName, bufferLen) == 0) 
        LOG_VERBOSE( L"friendly name: \"" << szDeviceName << L"\"");
...
{ 
    ...
    if (index == (uint16_t)(-1)) {
        ...
        LOG_VERBOSE( L"Default Communication endpoint device will be used");
    }
...
int32_t AudioDeviceWindowsCore::InitPlayout() 
{
    ...
    while(true)
    {
        ...
        if (InitSpeaker() == -1)
            LOG_WARNING( L"InitSpeaker() failed");
        ...
        if (hr == S_OK) {
            ...
            m_devicePlayBlockSize   = Wfx.nSamplesPerSec / 100; // Block size is the number of samples each channel in 10ms.
        ...
        if (FAILED(hr))
            LOG_ERROR( L"IAudioClient::Initialize() failed:");
        ...
        if (m_ptrAudioBuffer) {
            ...
            LOG_ERROR( L"AudioDeviceBuffer must be attached before streaming can start");
        }
        ...
        if (SUCCEEDED(hr))
            LOG_VERBOSE( L"IAudioClient::GetBufferSize() => " << bufferFrameCount << L" (<=> " << bufferFrameCount * m_playAudioFrameSize << L" bytes)");
        ...
        LOG_INFO( L"render side is now initialized");
...
int32_t AudioDeviceWindowsCore::InitRecordingDMO() 
{
    ...
    else 
    {
        ...
        LOG_VERBOSE( L"AudioDeviceBuffer must be attached before streaming can start");
    }
    ...
    LOG_VERBOSE( L"Capture side is now initialized");
...
int32_t AudioDeviceWindowsCore::InitRecording() 
{
    ...
    while(true)
    {
        ...
        if (InitMicrophone() == -1) {
            LOG_WARNING( L"InitMicrophone() failed");
        }
        ...
        if (SUCCEEDED(hr)) 
        {
            LOG_VERBOSE( L"Audio Engine's current capturing mix format:");
            // format type
            LOG_VERBOSE( L"wFormatTag     : 0x" << std::hex << pWfxIn->wFormatTag << std::dec << L" (" << pWfxIn->wFormatTag << L")");
            // number of channels (i.e. mono, stereo...)
            LOG_VERBOSE( L"nChannels      : " << pWfxIn->nChannels);
            // sample rate
            LOG_VERBOSE( L"nSamplesPerSec : " << pWfxIn->nSamplesPerSec);
            // for buffer estimation
            LOG_VERBOSE( L"nAvgBytesPerSec: " << pWfxIn->nAvgBytesPerSec);
            // block size of data
            LOG_VERBOSE( L"nBlockAlign    : " << pWfxIn->nBlockAlign);
            // number of bits per sample of mono data
            LOG_VERBOSE( L"wBitsPerSample : " << pWfxIn->wBitsPerSample);
            LOG_VERBOSE( L"cbSize         : " << pWfxIn->cbSize);
        }
        ...
        for (unsigned int freq = 0; freq < sizeof(freqs) / sizeof(freqs[0]); freq++) 
        {
            for (unsigned int chan = 0; chan < sizeof(m_recChannelsPrioList) / sizeof(m_recChannelsPrioList[0]); chan++) 
            {
                ...
                else 
                {
                    if (pWfxClosestMatch) 
                    {
                        LOG_INFO( L"nChannels=" << Wfx.Format.nChannels 
        ...
        if (hr == S_OK) 
        {
            ...
            LOG_VERBOSE( L"VoE selected this capturing format:");
            LOG_VERBOSE( L"wFormatTag        : 0x" << std::hex << Wfx.Format.wFormatTag << std::dec << L" (" << Wfx.Format.wFormatTag << L")");
            LOG_VERBOSE( L"nChannels         : " << Wfx.Format.nChannels);
            LOG_VERBOSE( L"nSamplesPerSec    : " << Wfx.Format.nSamplesPerSec);
            LOG_VERBOSE( L"nAvgBytesPerSec   : " << Wfx.Format.nAvgBytesPerSec);
            LOG_VERBOSE( L"nBlockAlign       : " << Wfx.Format.nBlockAlign);
            LOG_VERBOSE( L"wBitsPerSample    : " << Wfx.Format.wBitsPerSample);
            LOG_VERBOSE( L"cbSize            : " << Wfx.Format.cbSize);
            LOG_VERBOSE( L"Additional settings:");
            LOG_VERBOSE( L"_recAudioFrameSize: " << m_recAudioFrameSize);
            LOG_VERBOSE( L"_recBlockSize     : " << m_recBlockSize);
            LOG_VERBOSE( L"_recChannels      : " << m_recChannels);
        }
        ...
        if (SUCCEEDED(hr = m_ptrClientIn->GetBufferSize(&bufferFrameCount))) 
            LOG_VERBOSE( L"IAudioClient::GetBufferSize() => " << bufferFrameCount << L" (<=> " << bufferFrameCount * m_recAudioFrameSize << L" bytes)");
...
int32_t AudioDeviceWindowsCore::StartRecording() {
    ...
    {
        AutoLock critScoped(&m_critSect);
        ...
        if (m_builtInAecEnabled) 
        {
            ...
            if (!m_playing) 
            {
                ...
                LOG_ERROR( L"Playout must be started before recording when using" << L" the built-in AEC");
                return -1;
            }
        }
        ...
        if (m_hRecThread == NULL) 
        {
            LOG_ERROR( L"failed to create the recording thread");
            return -1;
        }
    ...
    {
        LOG_VERBOSE( L"capturing did not start up properly");
        return -1;
    }

    LOG_VERBOSE( L"capture audio stream has now started...");
...
int32_t AudioDeviceWindowsCore::StopRecording() 
{
    ...
    if (m_hRecThread == NULL) 
    {
        LOG_VERBOSE( L"no capturing stream is active => close down WASAPI only");
        m_ptrClientIn.Release();
        m_ptrCaptureClient.Release();
        m_recIsInitialized = false;
        m_recording = false;
    ...
    LOG_VERBOSE( L"closing down the webrtc_core_audio_capture_thread...");
    ...
    if (ret != WAIT_OBJECT_0) 
    {
        LOG_ERROR( L"failed to close down webrtc_core_audio_capture_thread");
        err = -1;
    }
    else 
        LOG_VERBOSE( L"webrtc_core_audio_capture_thread is now closed");
...
int32_t AudioDeviceWindowsCore::StartPlayout()
{
...

    {
        ...
        {
            LOG_ERROR( L"failed to create the playout thread");
            return -1;
        }
    ...
    if (ret != WAIT_OBJECT_0) 
    {
        LOG_WARNING( L"rendering did not start up properly");
        return -1;
    }

    m_playing = true;
    LOG_INFO( L"rendering audio stream has now started...");
...
int32_t AudioDeviceWindowsCore::StopPlayout()
{
...

    {
        ...
        if (m_hPlayThread == NULL)
        {
            LOG_VERBOSE( L"no rendering stream is active => close down WASAPI only");
            m_ptrClientOut.Release();
            m_ptrRenderClient.Release();
            m_playIsInitialized = false;
            m_playing = false;
        ...
        // stop the driving thread...
        LOG_VERBOSE( L"closing down the webrtc_core_audio_render_thread...");
    ...
    if (ret != WAIT_OBJECT_0)
    {
        ...
        LOG_ERROR( L"failed to close down webrtc_core_audio_render_thread");
    ...
    {
        AutoLock critScoped(&m_critSect);
        LOG_VERBOSE( L"webrtc_core_audio_render_thread is now closed");
        ...
        if (m_builtInAecEnabled && m_recording)
        {
            ...
            LOG_WARNING( L"Recording should be stopped before playout when using the" << L" built-in AEC");
        }
...
DWORD AudioDeviceWindowsCore::DoRenderThread()
{
    ...
    try
    {
        ...
        if (!comInit.succeeded()) 
        {
            LOG_ERROR( L"failed to initialize COM in render thread");
            throw S_FALSE;
        }
        ...
        if (_winSupportAvrt)
        {
            ...
            if (hMmTask)
            {
                if (FALSE == _PAvSetMmThreadPriority(hMmTask, AVRT_PRIORITY_CRITICAL))
                    LOG_WARNING( L"failed to boost play-thread using MMCSS");

                LOG_VERBOSE( L"render thread is now registered with MMCSS (taskIndex=" << taskIndex << L")");
            }
            else
            {
                LOG_WARNING( L"failed to enable MMCSS on render thread (err=" << GetLastError() << L")");
        ...
        if (FAILED(err))
            LOG_WARNING( L"failed to get IAudioClock interface from the IAudioClient");
        ...
        while (keepPlaying)
        {
            ...
            case WAIT_TIMEOUT:  // timeout notification
                LOG_WARNING( L"render event timed out after 0.5 seconds");
                throw err;
            default:  // unexpected error
                LOG_WARNING( L"unknown wait termination on render side");
                throw err;
            }
            ...
            while (keepPlaying)
            {
                ...
                {
                    _UnLock();
                    LOG_ERROR( L"output state has been modified during unlocked period");
                    throw err;
                }
                ...
                // buffer.
                UINT32 padding = 0;
                if (FAILED(err = m_ptrClientOut->GetCurrentPadding(&padding)))
                    throw err;
                ...
                for (uint32_t n = 0; n < n10msBuffers; n++)
                {
                    ...
                    if (m_ptrAudioBuffer)
                    {
                        ...
                        {
                            _UnLock();
                            LOG_ERROR( L"failed to read data from render client");
                            throw err;
                        }
                        ...
                        {
                            _UnLock();
                            LOG_ERROR( L"output state has been modified during unlocked" << L" period");
                            throw err;
                        }
    ...
    catch(HRESULT e)
    {
        ...
        if (keepPlaying)
        {
            ...
            LOG_ERROR( L"Playout error: rendering thread has ended pre-maturely");
        }
        else
            LOG_VERBOSE( L"_Rendering thread is now terminated properly");
    ...
    if (_winSupportAvrt) 
    {
        ...
        if (m_hMmTask) 
        {
            if (!_PAvSetMmThreadPriority(m_hMmTask, AVRT_PRIORITY_CRITICAL)) 
                LOG_WARNING( L"failed to boost rec-thread using MMCSS");

            LOG_VERBOSE( L"capture thread is now registered with MMCSS (taskIndex=" << taskIndex << L")");
        }
        else 
        {
            LOG_WARNING( L"failed to enable MMCSS on capture thread (err=" << GetLastError() << L")");
            _TraceCOMError(GetLastError());
        }
    }
...
DWORD AudioDeviceWindowsCore::DoCaptureThreadPollDMO() 
{
    ...
    if (!comInit.succeeded()) 
    {
        LOG_ERROR( L"failed to initialize COM in polling DMO thread");
        return 1;
    }
    ...
    while (keepRecording) 
    {
        ...
        case WAIT_TIMEOUT:  // timeout notification
            break;
        default:  // unexpected error
            LOG_WARNING( L"Unknown wait termination on capture side");
    ...
    if (FAILED(hr))
    {
        LOG_ERROR( L"Recording error: capturing thread has ended prematurely");
    }
    else
    {
        LOG_VERBOSE( L"Capturing thread is now terminated properly");
    }
...
DWORD AudioDeviceWindowsCore::DoCaptureThread() 
{
    ...
    try
    {
        ...
        if (!comInit.succeeded()) 
        {
            LOG_ERROR( L"failed to initialize COM in capture thread");
            throw S_FALSE;
        }
        ...
        {
            LOG_ERROR( L"input state has been modified before capture loop starts.");
            throw e;
        }
        ...
        LOG_VERBOSE( L"[CAPT] size of buffer       : " << bufferLength);
        ...
        LOG_VERBOSE( L"[CAPT] size of sync buffer  : " << syncBufferSize << L" [bytes]");
        ...
        LOG_VERBOSE( L"[CAPT] max stream latency   : " << (DWORD)latency << L" (" << (double)(latency / 10000.0) << L" ms)");
        ...
        LOG_VERBOSE( L"[CAPT] device period        : " << (DWORD)devPeriod << L" (" << (double)(devPeriod / 10000.0) << L" ms)");

        double extraDelayMS = (double)((latency + devPeriod) / 10000.0);
        LOG_VERBOSE( L"[CAPT] extraDelayMS         : " << extraDelayMS);

        double endpointBufferSizeMS = 10.0 * ((double)bufferLength / (double)m_recBlockSize);
        LOG_VERBOSE( L"[CAPT] endpointBufferSizeMS : " << endpointBufferSizeMS);
        ...
        while (keepRecording) 
        {
            ...
            case WAIT_OBJECT_0 + 1:  // _hCaptureSamplesReadyEvent
                break;
            case WAIT_TIMEOUT:  // timeout notification
                LOG_WARNING( L"capture event timed out after 0.5 seconds");
                throw e;
            default:  // unexpected error
                LOG_WARNING( L"unknown wait termination on capture side");
                throw e;
            ...
            while (keepRecording)
            {
                ...
                {
                    _UnLock();
                    LOG_ERROR( L"input state has been modified during unlocked period");
                    throw e;
                }
                ...
                if (SUCCEEDED(e))
                {
                    ...
                    if (flags & AUDCLNT_BUFFERFLAGS_SILENT)
                    {
                        ...
                        // actual data values.
                        LOG_WARNING( L"AUDCLNT_BUFFERFLAGS_SILENT");
                        pData = nullptr;
                    }
                    ...
                    // client.
                    LOG_ERROR( L"IAudioCaptureClient::GetBuffer returned AUDCLNT_E_BUFFER_ERROR, hr = 0x" << std::hex << e << std::dec);
                    throw e;;
                }
    ...
    catch(HRESULT e)
    {
        ...
        if (keepRecording)
        {
            ...
            LOG_ERROR( L"Recording error: capturing thread has ended pre-maturely");
        }
        else 
            LOG_VERBOSE( L"_Capturing thread is now terminated properly");
...
int32_t AudioDeviceWindowsCore::EnableBuiltInAEC(bool enable) 
{
    if (m_recIsInitialized) 
    {
        LOG_ERROR( L"Attempt to set Windows AEC with recording already initialized");
        return -1;
    }

    if (m_dmo == NULL) 
    {
        LOG_ERROR( L"Built-in AEC DMO was not initialized properly at create time");
        return -1;
    }
...
int AudioDeviceWindowsCore::SetDMOProperties() {
    ...
    LOG_VERBOSE( L"Capture device index: " << inDevIndex << L", render device index: " << outDevIndex);
...
int32_t AudioDeviceWindowsCore::_GetDefaultDeviceIndex(EDataFlow dir, ERole role, int* index) 
{
    ...
    if (!collection) {
        LOG_ERROR( L"Device collection not valid");
        return -1;
    }
    ...
    *index = -1;
    ...
    if (*index == -1) {
        LOG_ERROR( L"Unable to find collection index for default device");
        return -1;
    }
...
int32_t AudioDeviceWindowsCore::_GetDeviceName(IMMDevice* pDevice, LPWSTR pszBuffer, int bufferLen) 
{
    ...
    if (pDevice != NULL) 
    {
        ...
        if (FAILED(hr))
            LOG_ERROR( L"IMMDevice::OpenPropertyStore failed, hr = 0x" << std::hex << hr << std::dec);
    }
    ...
    if (SUCCEEDED(hr)) 
    {
        ...
        if (FAILED(hr))
            LOG_ERROR( L"IPropertyStore::GetValue failed, hr = 0x" << std::hex << hr << std::dec);
    }

    if ((SUCCEEDED(hr)) && (VT_EMPTY == varName.vt)) {
        hr = E_FAIL;
        LOG_ERROR( L"IPropertyStore::GetValue returned no value," << L" hr = 0x" << std::hex << hr << std::dec);
    }

    if ((SUCCEEDED(hr)) && (VT_LPWSTR != varName.vt)) {
        // The returned value is not a wide null terminated string.
        hr = E_UNEXPECTED;
        LOG_ERROR( L"IPropertyStore::GetValue returned unexpected)" << L" type, hr = 0x" << std::hex << hr << std::dec);
    }
...
int32_t AudioDeviceWindowsCore::_EnumerateEndpointDevicesAll(EDataFlow dataFlow) const 
{
    ...
    if (dataFlow == eRender)
    {
        LOG_VERBOSE( L"#rendering endpoint devices (counting all): " << count);
    }
    else if (dataFlow == eCapture)
    {
        LOG_VERBOSE( L"#capturing endpoint devices (counting all): " << count);
    }
    ...
    for (ULONG i = 0; i < count; i++) {
        ...
        LOG_VERBOSE( L"ID string    : " << pwszID);
        ...
        LOG_VERBOSE( L"friendly name: \"" << varName.pwszVal << L"\"");
        ...
        if (dwState & DEVICE_STATE_ACTIVE)
            LOG_VERBOSE( L"state (0x" << std::hex << dwState << std::dec << L")  : *ACTIVE*");
        if (dwState & DEVICE_STATE_DISABLED)
            LOG_VERBOSE( L"state (0x" << std::hex << dwState << std::dec << L")  : DISABLED");
        if (dwState & DEVICE_STATE_NOTPRESENT)
            LOG_VERBOSE( L"state (0x" << std::hex << dwState << std::dec << L")  : NOTPRESENT");
        if (dwState & DEVICE_STATE_UNPLUGGED)
            LOG_VERBOSE( L"state (0x" << std::hex << dwState << std::dec << L")  : UNPLUGGED");
        ...
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME) // The audio endpoint device supports a hardware volume control
            LOG_VERBOSE( L"hwmask (0x" << std::hex << dwHwSupportMask << std::dec << L") : HARDWARE_SUPPORT_VOLUME");
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_MUTE) // The audio endpoint device supports a hardware mute control
            LOG_VERBOSE( L"hwmask (0x" << std::hex << dwHwSupportMask << std::dec << L") : HARDWARE_SUPPORT_MUTE");
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_METER) // The audio endpoint device supports a hardware peak meter
            LOG_VERBOSE( L"hwmask (0x" << std::hex << dwHwSupportMask << std::dec << L") : HARDWARE_SUPPORT_METER");
        ...
        LOG_VERBOSE( L"#channels    : " << nChannelCount);
        ...
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME) 
        {
            ...
            LOG_VERBOSE( L"volume range : " << fLevelMinDB << L" (min), " << fLevelMaxDB << L" (max), " << fVolumeIncrementDB << L" (inc) [dB]");
            ...
            LOG_VERBOSE( L"volume steps : " << nStep << L" (nStep), " << nStepCount << L" (nStepCount)");
        }
    Next:
        if (FAILED(hr))
            LOG_VERBOSE( L"Error when logging device information");
...
void AudioDeviceWindowsCore::_TraceCOMError(HRESULT hr) const { 
    ...
    LOG_ERROR( L"Core Audio method failed (hr=" << hr << L")");
